...
Where QwkSearch sits next to the closed research assistants. ✅ means the
feature is there and works the way the row describes, ⚠️ that it exists but is
partial or indirect, ❌ that it isn't offered.
...
<Callout type="info">
Competitor capabilities change often and this table is maintained by hand —
check the vendor's own docs before relying on a row. The QwkSearch column is
the one you can check yourself — every claim in it is documented on these pages
and implemented in the
[repository](https://github.com/OpenSourceAGI/qwksearch-research-agent).
</Callout>
